      ...
      <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-gray-950 text-white">
        ...
        <div className="container mx-auto px-6 py-8">
          <div className="text-center py-12">
            ...
            <button 
              onClick={refreshChallenges}
              className="bg-white/10 backdrop-blur-sm text-white px-6 py-3 rounded-full font-semibold hover:bg-white/20 transition-all border border-white/20 flex items-center space-x-2 mx-auto mb-8"
              disabled={refreshing}
            >
              <RefreshCw className={`w-5 h-5 ${refreshing ? 'animate-spin' : ''}`} />
              <span>{refreshing ? 'Refreshing...' : 'Refresh Challenges'}</span>
    ...
    <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-gray-950 text-white">
      ...
      <div className="container mx-auto px-6 py-8">
        <div className="flex justify-end mb-4">
          <button 
            onClick={refreshChallenges}
            className="bg-white/10 backdrop-blur-sm text-white px-4 py-2 rounded-full font-semibold hover:bg-white/20 transition-all border border-white/20 flex items-center space-x-2"
            disabled={refreshing}
          >
            <RefreshCw className={`w-4 h-4 ${refreshing ? 'animate-spin' : ''}`} />
            <span>{refreshing ? 'Refreshing...' : 'Refresh'}</span>
          ...
          <div className="lg:col-span-2 space-y-8">
            <div className="bg-white/5 backdrop-blur-md rounded-2xl shadow-xl overflow-hidden border border-white/10">
              <div className="bg-gradient-to-r from-orange-900 to-orange-700 text-white p-6">
                ...
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <div className="bg-white/20 backdrop-blur-sm rounded-lg p-3 text-center">
                    <Target className="w-6 h-6 mx-auto mb-1 text-amber-300" />
                    <div className="text-2xl font-bold">{currentChallenge.maxScore}</div>
                    <div className="text-sm text-white/80">Max Points</div>
                  </div>
                  <div className="bg-white/20 backdrop-blur-sm rounded-lg p-3 text-center">
                    <Award className="w-6 h-6 mx-auto mb-1 text-red-300" />
                    <div className="text-2xl font-bold">
                      {userStats.currentRank ? `#${userStats.currentRank}` : '-'}
                    </div>
                    ...
                    {currentChallenge.rules?.map((rule, index) => (
                      <div key={index} className="flex items-start space-x-3">
                        <div className="w-6 h-6 bg-red-900/50 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
                          <span className="text-xs font-semibold text-red-300">{index + 1}</span>
                        </div>
                ...
                <div className="space-y-4">
                  {!userJoined ? (
                    <div className="space-y-3">
                      <button 
                        onClick={handleJoinChallenge}
                        className="w-full bg-gradient-to-r from-orange-600 to-amber-500 text-white py-4 rounded-xl font-semibold text-lg hover:shadow-lg transform hover:scale-[1.02] transition-all duration-300"
                      >
                        <span className="flex items-center justify-center space-x-2">
                      ...
                      <button disabled className="w-full bg-gray-700/50 text-gray-500 py-4 rounded-xl font-semibold text-lg cursor-not-allowed">
                        <span className="flex items-center justify-center space-x-2">
                          <Upload className="w-5 h-5" />
                          <span>Submit Your Proof</span>
                        </span>
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center space-x-2 text-green-400 bg-green-900/50 p-3 rounded-lg border border-green-500/30">
                        <CheckCircle className="w-5 h-5" />
                        <span className="font-medium">Challenge Joined! Time to compete.</span>
                      </div>
                      
                      <button 
                        onClick={handleSubmitProof}
                        className="w-full bg-gradient-to-r from-green-500 to-teal-500 text-white py-4 rounded-xl font-semibold text-lg hover:shadow-lg transform hover:scale-[1.02] transition-all duration-300 animate-pulse"
                      >
                        <span className="flex items-center justify-center space-x-2">
                          <Upload className="w-5 h-5" />
                          <span>Submit Your Proof</span>
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center space-x-2 text-amber-400 bg-amber-900/50 p-3 rounded-lg border border-amber-500/30">
                        <CheckCircle className="w-5 h-5" />
                        <span className="font-medium">Submission Complete! Score: {userSubmission.score}</span>
                      </div>
                      <button disabled className="w-full bg-gray-700/50 text-gray-500 py-4 rounded-xl font-semibold text-lg cursor-not-allowed">
            ...
            {previousChallenges.length > 0 && (
              <div className="bg-white/5 backdrop-blur-md rounded-2xl shadow-lg p-6 border border-white/10">
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  {previousChallenges.map((challenge) => (
                    <div key={challenge._id} className="bg-gray-900/50 border border-white/10 rounded-lg p-4 hover:border-amber-400 transition-colors">
                      <div className="flex justify-between items-start mb-3">
                ...
                <div className="mt-4 text-center">
                  <button 
                    onClick={() => navigate('/challenges')}
                    className="text-amber-400 hover:text-amber-300 font-medium text-sm hover:underline"
                  >
                    View All Challenges →
                  </button>
                </div>
          ...
          <div className="space-y-6">
            <div className="bg-white/5 backdrop-blur-md rounded-2xl shadow-lg p-6 border border-white/10">
              ...
              <div className="space-y-4 text-gray-300">
                <div className="flex justify-between items-center"><span className="font-medium">Current Rank</span><span className="text-2xl font-bold text-red-400">{userStats.currentRank ? `#${userStats.currentRank}` : '-'}</span></div>
                <div className="flex justify-between items-center"><span className="font-medium">Total Score</span><span className="text-2xl font-bold text-green-400">{userStats.totalScore}</span></div>
                <div className="flex justify-between items-center"><span className="font-medium">Challenges Won</span><span className="text-xl font-semibold text-amber-400">{userStats.challengesWon}</span></div>
                <div className="flex justify-between items-center"><span className="font-medium">Total Submissions</span><span className="text-xl font-semibold text-gray-100">{userStats.totalSubmissions}</span></div>
              </div>
            </div>
                ...
                <div className="space-y-4">
                  {upcomingChallenges.map((challenge) => (
                    <div key={challenge._id} className="bg-gray-900/50 border border-white/10 rounded-lg p-4 hover:border-amber-400 transition-colors">
                      ...
                      <div className="flex justify-between items-center text-sm">
                        <span className="text-amber-400 font-medium">Starts in {formatTimeRemaining(challenge.startTime)}</span>
                        <span className="text-gray-500 flex items-center gap-1"><Users size={14}/> {challenge.totalParticipants || 0}</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="bg-gradient-to-r from-orange-900 to-orange-700 rounded-2xl p-6 text-white">
              <h3 className="text-lg font-bold mb-4">Quick Actions</h3>
              <div className="space-y-3">
                <button onClick={() => navigate('/leaderboard')} className="w-full bg-white/20 backdrop-blur-sm py-3 rounded-lg font-medium hover:bg-white/30 transition-colors">View Leaderboard</button>
                <button onClick={() => navigate('/challenges')} className="w-full bg-white/20 backdrop-blur-sm py-3 rounded-lg font-medium hover:bg-white/30 transition-colors">All Challenges</button>
                {user?.role === 'admin' && (
                  <button onClick={() => navigate('/admin')} className="w-full bg-white/20 backdrop-blur-sm py-3 rounded-lg font-medium hover:bg-white/30 transition-colors">Admin Panel</button>
                )}
              </div>
            </div>
          </div>
